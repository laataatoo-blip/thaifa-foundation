...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $result = $auth->requestPasswordReset($identity);
        $success = 'หากพบข้อมูลบัญชีในระบบ เราได้สร้างลิงก์รีเซ็ตรหัสผ่านให้แล้ว';
...
<head>
    ...
    <title>ลืมรหัสผ่าน - THAIFA Foundation</title>
...
<main class="pt-[120px] min-h-screen flex items-center justify-center px-4 py-10">
    <div class="w-full max-w-md bg-white border border-border/70 rounded-2xl shadow-xl p-8">
        <h1 class="text-2xl text-primary font-semibold text-center mb-2">ลืมรหัสผ่าน</h1>
        <p class="text-center text-sm text-foreground/60 mb-6">กรอกอีเมลหรือเบอร์โทรที่เคยสมัครไว้</p>
        ...
        <?php if ($success !== ''): ?>
            ...
            <?php if ($resetLink !== ''): ?>
                <div class="mb-4 rounded-lg border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-800">
                    ลิงก์รีเซ็ต (สำหรับทดสอบระบบ):
                    <a class="underline break-all" href="<?= h($resetLink) ?>"><?= h($resetLink) ?></a>
                </div>
            <?php endif; ?>
        <?php endif; ?>
        ...
        <form method="post" class="space-y-4">
            <div>
                <label class="block mb-1 text-sm">อีเมลหรือเบอร์โทร</label>
                <input type="text" name="identity" value="<?= h($identity) ?>" required class="w-full px-4 py-3 border border-border rounded-lg">
            </div>
            <button type="submit" class="w-full bg-primary text-white py-3 rounded-lg hover:bg-primary/90">ขอลิงก์รีเซ็ตรหัสผ่าน</button>
        </form>

        <div class="mt-4 text-center text-sm"><a href="login.php" class="text-primary hover:underline">กลับไปหน้าเข้าสู่ระบบ</a></div>
    </div>
</main>
